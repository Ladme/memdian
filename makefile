all: src/memthick.c src/wdcalc.c src/wdmap.c src/memplanes.c
	make memthick groan=${groan}
	make wdcalc groan=${groan}
	make wdmap groan=${groan}
	make memplanes groan=${groan}
...
memplanes: src/memplanes.c
	gcc src/memplanes.c -I$(groan) -L$(groan) -D_POSIX_C_SOURCE=200809L -o memplanes -lgroan -lm -std=c99 -pedantic -Wall -Wextra -O3 -march=native
...
install:
	...
	if [ -f memplanes ]; then cp memplanes ${HOME}/.local/bin; fi
